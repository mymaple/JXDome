...
<body class="no-skin">
	...
	<script type="text/javascript">
		...
		//删除
		function toDelete(mapleDetailId){
			bootbox.confirm("确定要删除吗?", function(result) {
				if(result) {
					top.jzts();
					var url = "<%=basePath%>background/mapleDetail/toDelete.do?mapleDetailId="+mapleDetailId+"&tm="+new Date().getTime();
				...
				if(result) {
					...
					if(str==''){
						...
						if(msg == '确定要删除选中的数据吗?'){
							...
							$.ajax({
								type: "POST",
								url: '<%=basePath%>background/mapleDetail/toBatchDelete.do?tm='+new Date().getTime(),
						    	data: {ids:str},
								dataType:'json',
		...
		function toExportExcel(){
			window.location.href='<%=basePath%>background/mapleDetail/toExportExcel.do';
		}
