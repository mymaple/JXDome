...
<body class="no-skin">
			...
			<div class="main-content-inner">
				<div class="page-content">
					<div class="row">
						...
						<!-- 检索  -->
						<form action="background/supplier/list.do" method="post" name="supplierForm" id="supplierForm">
						...
						<table id="simple-table" class="table table-striped table-bordered table-hover" style="margin-top:5px;">	
							...
							<!-- 开始循环 -->	
							<c:choose>
								<c:when test="${not empty comSupplierList}">
									<c:if test="${RIGHTS.sele}">
									<c:forEach items="${comSupplierList}" var="comSupplier" varStatus="vs">
										<tr>
											<td class='center'>
											...
											<td class='center'>${comSupplier.linkman}</td>
											<td class='center'>${comSupplier.linkPhone}</td>
											<td class='center'>${comSupplier.remarks}</td>
											<td class='center'>${comSupplier.orderNum}</td>
											<td class="center">
												...
												<div class="hidden-sm hidden-xs btn-group">
													...
													<a class="btn btn-xs btn-danger" onclick="toDelete('${comSupplier.supplierId}');">
	...
	<script type="text/javascript">
		...
		//删除
		function toDelete(supplierId){
			bootbox.confirm("确定要删除吗?", function(result) {
				if(result) {
					top.jzts();
					var url = "<%=basePath%>background/supplier/toDelete.do?supplierId="+supplierId+"&tm="+new Date().getTime();
					$.get(url,function(data){
						if(data.resultCode == "success"){
							nextPage('${bgPage.currentPage}');
						}
				...
				if(result) {
					...
					if(str==''){
						...
						if(msg == '确定要删除选中的数据吗?'){
							...
							$.ajax({
								type: "POST",
								url: '<%=basePath%>background/supplier/toBatchDelete.do?tm='+new Date().getTime(),
						    	data: {ids:str},
								dataType:'json',
		...
		function toExportExcel(){
			window.location.href='<%=basePath%>background/supplier/toExportExcel.do';
		}
		...
		function toUploadExcel(){
			 ...
			 diag.URL = '<%=basePath%>background/supplier/toUploadExcel.do';
			 diag.Width = 300;
			 diag.Height = 150;
			 diag.CancelEvent = function(){ //关闭事件
				 if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
