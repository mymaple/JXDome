...
<body class="no-skin">
			...
			<div class="main-content-inner">
				<div class="page-content">
					<div class="row">
						...
						<!-- 检索  -->
						<form action="background/productCategory/list.do" method="post" name="productCategoryForm" id="productCategoryForm">
						...
						<table id="simple-table" class="table table-striped table-bordered table-hover" style="margin-top:5px;">	
							...
							<!-- 开始循环 -->	
							<c:choose>
								<c:when test="${not empty comProductCategoryList}">
									<c:if test="${RIGHTS.sele}">
									<c:forEach items="${comProductCategoryList}" var="comProductCategory" varStatus="vs">
										<tr>
											<td class='center'>
											...
											<td class="center">
												...
												<div class="hidden-sm hidden-xs btn-group">
													...
													<a class="btn btn-xs btn-danger" onclick="toDelete('${comProductCategory.productCategoryId}');">
	...
	<script type="text/javascript">
		...
		//删除
		function toDelete(productCategoryId){
			bootbox.confirm("确定要删除吗?", function(result) {
				if(result) {
					top.jzts();
					var url = "<%=basePath%>background/productCategory/toDelete.do?productCategoryId="+productCategoryId+"&tm="+new Date().getTime();
		...
		//修改
		function toEdit(productCategoryId){
			 top.jzts();
			 var diag = new top.Dialog();
			 diag.Drag=true;
			 diag.Title ="编辑";
				...
				if(result) {
					...
					if(str==''){
						...
						if(msg == '确定要删除选中的数据吗?'){
							...
							$.ajax({
								type: "POST",
								url: '<%=basePath%>background/productCategory/toBatchDelete.do?tm='+new Date().getTime(),
						    	data: {ids:str},
								dataType:'json',
		...
		function toExportExcel(){
			window.location.href='<%=basePath%>background/productCategory/toExportExcel.do';
		}
		...
		function toUploadExcel(){
			 ...
			 diag.URL = '<%=basePath%>background/productCategory/toUploadExcel.do';
			 diag.Width = 300;
			 diag.Height = 150;
			 diag.CancelEvent = function(){ //关闭事件
				 if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
