...
<body class="no-skin">
			...
			<div class="main-content-inner">
				<div class="page-content">
					<div class="row">
						...
						<!-- 检索  -->
						<form action="background/integralNote/list.do" method="post" name="integralNoteForm" id="integralNoteForm">
							...
							<c:choose>
								<c:when test="${not empty comIntegralNoteList}">
									...
									<c:forEach items="${comIntegralNoteList}" var="comIntegralNote" varStatus="vs">
										<tr>
											<td class='center'>
												<label class="pos-rel"><input type='checkbox' name='ids' value="${comIntegralNote.integralNoteId}" class="ace" /><span class="lbl"></span></label>
											</td>
											...
											<td class='center'>${comIntegralNote.orderNum}</td>
											<td class="center">
												...
												<div class="hidden-sm hidden-xs btn-group">
													...
													<a class="btn btn-xs btn-success" title="编辑" onclick="toEdit('${comIntegralNote.integralNoteId}');">
														<i class="ace-icon fa fa-pencil-square-o bigger-120" title="编辑"></i>
													</a>
													</c:if>
													<c:if test="${RIGHTS.del }">
													<a class="btn btn-xs btn-danger" onclick="toDelete('${comIntegralNote.integralNoteId}');">
	...
	<script type="text/javascript">
		...
		function toAdd(){
			 ...
			 diag.Drag=true;
			 diag.Title ="新增";
			 diag.URL = "<%=basePath%>background/integralNote/toAdd.do";
			 diag.Width = 450;
			 diag.Height = 355;
		...
		//删除
		function toDelete(integralNoteId){
			bootbox.confirm("确定要删除吗?", function(result) {
				if(result) {
					top.jzts();
					var url = "<%=basePath%>background/integralNote/toDelete.do?integralNoteId="+integralNoteId+"&tm="+new Date().getTime();
		...
		//修改
		function toEdit(integralNoteId){
			 top.jzts();
			 var diag = new top.Dialog();
			 diag.Drag=true;
			 diag.Title ="编辑";
			 diag.URL = "<%=basePath%>background/integralNote/toEdit.do?integralNoteId="+integralNoteId+"&tm="+new Date().getTime();
			 diag.Width = 450;
			 diag.Height = 355;
			 diag.Modal = true;				//有无遮罩窗口
			 diag. ShowMaxButton = true;	//最大化按钮
				...
				if(result) {
					...
					if(str==''){
						...
						if(msg == '确定要删除选中的数据吗?'){
							...
							$.ajax({
								type: "POST",
								url: '<%=basePath%>background/integralNote/toBatchDelete.do?tm='+new Date().getTime(),
						    	data: {ids:str},
								dataType:'json',
		...
		function toExportExcel(){
			window.location.href='<%=basePath%>background/integralNote/toExportExcel.do';
		}
		...
		function toUploadExcel(){
			 ...
			 diag.URL = '<%=basePath%>background/integralNote/toUploadExcel.do';
			 diag.Width = 300;
			 diag.Height = 150;
			 diag.CancelEvent = function(){ //关闭事件
				 if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
