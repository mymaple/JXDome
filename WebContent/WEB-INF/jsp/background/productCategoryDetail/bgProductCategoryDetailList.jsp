...
<body class="no-skin">
	...
	<h1>
		<a href="<%=basePath%>background/productCategoryDetail/list.do">管理</a>
	</h1>
	</div><!-- /.page-header -->
			...
			<div class="main-content-inner">
				<div class="page-content">
					<div class="row">
						...
						<!-- 检索  -->
						<form action="background/productCategoryDetail/list.do" method="post" name="productCategoryDetailForm" id="productCategoryDetailForm">
						<input type="hidden" name="productCategoryId" id="productCategoryId" value="${pd.productCategoryId }"/>
						...
						<table id="simple-table" class="table table-striped table-bordered table-hover" style="margin-top:5px;">	
							...
							<!-- 开始循环 -->	
							<c:choose>
								<c:when test="${not empty comProductCategoryDetailList}">
									<c:if test="${RIGHTS.sele}">
									<c:forEach items="${comProductCategoryDetailList}" var="comProductCategoryDetail" varStatus="vs">
										<tr>
											<td class='center'>
											...
											<td class="center">
												...
												<div class="hidden-sm hidden-xs btn-group">
													...
													<a class="btn btn-xs btn-danger" onclick="toDelete('${comProductCategoryDetail.productCategoryDetailId}');">
	...
	<script type="text/javascript">
		...
		function toAdd(){
			 ...
			 diag.Drag=true;
			 diag.Title ="新增";
			 diag.URL = "<%=basePath%>background/productCategoryDetail/toAdd.do?productCategoryId=${pd.productCategoryId }";
			 diag.Width = 450;
			 diag.Height = 355;
		...
		//删除
		function toDelete(productCategoryDetailId){
			bootbox.confirm("确定要删除吗?", function(result) {
				if(result) {
					top.jzts();
					var url = "<%=basePath%>background/productCategoryDetail/toDelete.do?productCategoryDetailId="+productCategoryDetailId+"&tm="+new Date().getTime();
					$.get(url,function(data){
						if(data.resultCode == "success"){
							nextPage('${bgPage.currentPage}');
						}
		...
		//修改
		function toEdit(productCategoryDetailId){
			 top.jzts();
			 var diag = new top.Dialog();
			 diag.Drag=true;
			 diag.Title ="编辑";
			 diag.URL = "<%=basePath%>background/productCategoryDetail/toEdit.do?productCategoryDetailId="+productCategoryDetailId+"&tm="+new Date().getTime();
			 diag.Width = 450;
			 diag.Height = 355;
			 diag.Modal = true;				//有无遮罩窗口
			 diag. ShowMaxButton = true;	//最大化按钮
				...
				if(result) {
					...
					if(str==''){
						...
						if(msg == '确定要删除选中的数据吗?'){
							...
							$.ajax({
								type: "POST",
								url: '<%=basePath%>background/productCategoryDetail/toBatchDelete.do?tm='+new Date().getTime(),
						    	data: {ids:str},
								dataType:'json',
		...
		function toExportExcel(){
			window.location.href='<%=basePath%>background/productCategoryDetail/toExportExcel.do';
		}
		...
		function toUploadExcel(){
			 ...
			 diag.URL = '<%=basePath%>background/productCategoryDetail/toUploadExcel.do?pId=${pd.productCategoryId }';
			 diag.Width = 300;
			 diag.Height = 150;
			 diag.CancelEvent = function(){ //关闭事件
				 if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
