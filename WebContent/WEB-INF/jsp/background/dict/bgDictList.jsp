...
<body class="no-skin">
							...
							<c:choose>
								<c:when test="${not empty comDictList}">
									...
									<c:forEach items="${comDictList}" var="comDict" varStatus="vs">
										<tr>
											<td class='center'>
												<label class="pos-rel"><input type='checkbox' name='ids' value="${comDict.dictId}-${comDict.parentId}" class="ace" /><span class="lbl"></span></label>
											</td>
											...
											<td class="center">
												...
												<div class="hidden-sm hidden-xs btn-group">
													...
													<a class="btn btn-xs btn-success" title="编辑" onclick="toEdit('${comDict.dictId}', '${comDict.parentId}');">
														<i class="ace-icon fa fa-pencil-square-o bigger-120" title="编辑"></i>
													</a>
													</c:if>
													<c:if test="${RIGHTS.del }">
													<a class="btn btn-xs btn-danger" onclick="toDelete('${comDict.dictId}', '${comDict.parentId}');">
	...
	<script type="text/javascript">
		...
		function toAdd(){
			 ...
			 diag.Drag=true;
			 diag.Title ="新增";
			 diag.URL = "<%=basePath%>background/dict/toAdd.do";
			 diag.Width = 450;
			 diag.Height = 355;
		...
		//删除
		function toDelete(dictId, parentId){
			bootbox.confirm("确定要删除吗?", function(result) {
				if(result) {
					top.jzts();
					var url = "<%=basePath%>background/dict/toDelete.do?dictId="+dictId+"&parentId="+parentId+"&tm="+new Date().getTime();
		...
		//修改
		function toEdit(dictId, parentId){
			 top.jzts();
			 var diag = new top.Dialog();
			 diag.Drag=true;
			 diag.Title ="编辑";
			 diag.URL = "<%=basePath%>background/dict/toEdit.do?dictId="+dictId+"&parentId="+parentId+"&tm="+new Date().getTime();
			 diag.Width = 450;
			 diag.Height = 355;
			 diag.Modal = true;				//有无遮罩窗口
			 diag. ShowMaxButton = true;	//最大化按钮
		...
		function toUploadExcel(){
			 ...
			 diag.URL = '<%=basePath%>background/dict/toUploadExcel.do';
			 diag.Width = 300;
			 diag.Height = 150;
			 diag.CancelEvent = function(){ //关闭事件
				 if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
