			...
			<div class="main-content-inner">
				<div class="page-content">
					<div class="row">
						...
						<!-- 检索  -->
						<form action="background/receiveAddress/list.do" method="post" name="receiveAddressForm" id="receiveAddressForm">
								...
								<c:when test="${not empty comReceiveAddressList}">
									...
									<c:forEach items="${comReceiveAddressList}" var="comReceiveAddress" varStatus="vs">
										<tr>
											...
											<td class='center'>${comReceiveAddress.city}</td>
											...
											<td class="center">
												...
												<div class="hidden-sm hidden-xs btn-group">
													...
													<a class="btn btn-xs btn-danger" onclick="toDelete('${comReceiveAddress.receiveAddressId}');">
	...
	<script type="text/javascript">
		...
		function toDelete(receiveAddressId){
			bootbox.confirm("确定要删除吗?", function(result) {
				if(result) {
					top.jzts();
					var url = "<%=basePath%>background/receiveAddress/toDelete.do?receiveAddressId="+receiveAddressId+"&tm="+new Date().getTime();
					$.get(url,function(data){
						if(data.resultCode == "success"){
							nextPage('${bgPage.currentPage}');
						}
				...
				if(result) {
					...
					if(str==''){
						...
						if(msg == '确定要删除选中的数据吗?'){
							...
							$.ajax({
								type: "POST",
								url: '<%=basePath%>background/receiveAddress/toBatchDelete.do?tm='+new Date().getTime(),
						    	data: {ids:str},
								dataType:'json',
		...
		function toExportExcel(){
			window.location.href='<%=basePath%>background/receiveAddress/toExportExcel.do';
		}
		...
		function toUploadExcel(){
			 ...
			 diag.URL = '<%=basePath%>background/receiveAddress/toUploadExcel.do';
			 diag.Width = 300;
			 diag.Height = 150;
			 diag.CancelEvent = function(){ //关闭事件
				 if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
