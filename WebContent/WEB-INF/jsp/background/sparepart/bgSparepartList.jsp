...
<body class="no-skin">
			...
			<div class="main-content-inner">
				<div class="page-content">
					<div class="row">
						...
						<!-- 检索  -->
						<form action="background/sparepart/list.do" method="post" name="sparepartForm" id="sparepartForm">
						...
						<table id="simple-table" class="table table-striped table-bordered table-hover" style="margin-top:5px;">	
							<thead>
								<tr>
									...
									<th class="center">零配件代号</th>
									<th class="center">零配件名称</th>
									<th class="center">零配件类型</th>
							...
							<!-- 开始循环 -->	
							<c:choose>
								<c:when test="${not empty comSparepartList}">
									<c:if test="${RIGHTS.sele}">
									<c:forEach items="${comSparepartList}" var="comSparepart" varStatus="vs">
										<tr>
											<td class='center'>
												<label class="pos-rel"><input type='checkbox' name='ids' value="${comSparepart.sparepartId}" class="ace" /><span class="lbl"></span></label>
											</td>
											<td class='center' style="width: 30px;">${vs.index+1}</td>
											<td class='center'>${comSparepart.sparepartCode}</td>
											<td class='center'>${comSparepart.sparepartName}</td>
											<td class='center'><param:display type="com_sparepartType" value="${comSparepart.sparepartType}"/></td>
											<td class='center'>${comSparepart.allIntegral}</td>
											...
											<td class="center">
												...
												<div class="hidden-sm hidden-xs btn-group">
													...
													<a class="btn btn-xs btn-success" title="编辑" onclick="toEdit('${comSparepart.sparepartId}');">
														<i class="ace-icon fa fa-pencil-square-o bigger-120" title="编辑"></i>
													</a>
													</c:if>
													<c:if test="${RIGHTS.del }">
													<a class="btn btn-xs btn-danger" onclick="toDelete('${comSparepart.sparepartId}');">
	...
	<script type="text/javascript">
		...
		//删除
		function toDelete(sparepartId){
			bootbox.confirm("确定要删除吗?", function(result) {
				if(result) {
					top.jzts();
					var url = "<%=basePath%>background/sparepart/toDelete.do?sparepartId="+sparepartId+"&tm="+new Date().getTime();
				...
				if(result) {
					...
					if(str==''){
						...
						if(msg == '确定要删除选中的数据吗?'){
							...
							$.ajax({
								type: "POST",
								url: '<%=basePath%>background/sparepart/toBatchDelete.do?tm='+new Date().getTime(),
						    	data: {ids:str},
								dataType:'json',
		...
		function toExportExcel(){
			window.location.href='<%=basePath%>background/sparepart/toExportExcel.do';
		}
		...
		function toUploadExcel(){
			 ...
			 diag.URL = '<%=basePath%>background/sparepart/toUploadExcel.do';
			 diag.Width = 300;
			 diag.Height = 150;
			 diag.CancelEvent = function(){ //关闭事件
				 if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
