...
<body class="no-skin">
						...
						<table id="simple-table" class="table table-striped table-bordered table-hover" style="margin-top:5px;">	
							...
							<c:choose>
								<c:when test="${not empty bgUserList}">
									<c:if test="${RIGHTS.sele}">
									<c:forEach items="${bgUserList}" var="bgUser" varStatus="vs">
										<tr>
											<td class='center'>
												<label class="pos-rel"><input type='checkbox' name='ids' value="${bgUser.userId}" class="ace" /><span class="lbl"></span></label>
											</td>
											...
											<td class='center'>${bgUser.orderNum}</td>
											<td class="center">
												...
												<div class="hidden-sm hidden-xs btn-group">
													...
													<a class="btn btn-xs btn-success" title="编辑" onclick="toEdit('${bgUser.userId}');">
														<i class="ace-icon fa fa-pencil-square-o bigger-120" title="编辑"></i>
													</a>
													</c:if>
													<c:if test="${RIGHTS.del }">
													<a class="btn btn-xs btn-danger" onclick="toDelete('${bgUser.userId}');">
	...
	<script type="text/javascript">
		...
		function toExportExcel(){
			window.location.href='<%=basePath%>background/user/toExportExcel.do';
		}
