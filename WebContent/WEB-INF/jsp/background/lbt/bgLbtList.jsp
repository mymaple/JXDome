...
<body class="no-skin">
			...
			<div class="main-content-inner">
				<div class="page-content">
					<div class="row">
						...
						<!-- 检索  -->
						<form action="background/lbt/list.do" method="post" name="lbtForm" id="lbtForm">
							...
							<c:choose>
								<c:when test="${not empty comLbtList}">
									...
									<c:forEach items="${comLbtList}" var="comLbt" varStatus="vs">
										<tr>
											<td class='center'>
												<label class="pos-rel"><input type='checkbox' name='ids' value="${comLbt.lbtId}" class="ace" /><span class="lbl"></span></label>
											</td>
											<c:if test="${comLbt.effective == '00'}">
											...
											<td class="center">
												...
												<div class="hidden-sm hidden-xs btn-group">
													...
													<c:if test="${comLbt.effective == '00'}">
													<a class="btn btn-xs btn-info" onclick="changeEffective('01','${comLbt.lbtId}');">使生效</a>
													</c:if>
													<c:if test="${comLbt.effective != '00'}">
													<a class="btn btn-xs btn-grey" onclick="changeEffective('00','${comLbt.lbtId}');">使失效</a>
													</c:if>
													
													<a class="btn btn-xs btn-success" title="编辑" onclick="toEdit('${comLbt.lbtId}');">
														<i class="ace-icon fa fa-pencil-square-o bigger-120" title="编辑"></i>
													</a>
													</c:if>
													<c:if test="${RIGHTS.del }">
													<a class="btn btn-xs btn-danger" onclick="toDelete('${comLbt.lbtId}');">
	...
	<script type="text/javascript">
		...
		function changeEffective(flag,lbtId){
			var firm = "确定要生效吗?";
			if("00"==flag){
				firm = "确定要失效吗?"
			}
			bootbox.confirm(firm, function(result) {
				if(result) {
					top.jzts();
					var url = "<%=basePath%>background/lbt/changeEffective.do?flag="+flag+"&lbtId="+lbtId+"&tm="+new Date().getTime();
		...
		function changeStatus(flag,lbtId){
			var firm = "确定要生效吗?";
			if("00"==flag){
				firm = "确定要失效吗?"
			}
			bootbox.confirm(firm, function(result) {
				if(result) {
					top.jzts();
					var url = "<%=basePath%>background/lbt/changeStatus.do?flag="+flag+"&lbtId="+lbtId+"&tm="+new Date().getTime();
					$.get(url,function(data){
						if(data.resultCode == "success"){
							nextPage('${bgPage.currentPage}');
						}
		...
		function toAdd(){
			 ...
			 diag.Drag=true;
			 diag.Title ="新增";
			 diag.URL = "<%=basePath%>background/lbt/toAdd.do";
			 diag.Width = 450;
			 diag.Height = 500;
		...
		//删除
		function toDelete(lbtId){
			bootbox.confirm("确定要删除吗?", function(result) {
				if(result) {
					top.jzts();
					var url = "<%=basePath%>background/lbt/toDelete.do?lbtId="+lbtId+"&tm="+new Date().getTime();
					$.get(url,function(data){
						if(data.resultCode == "success"){
							nextPage('${bgPage.currentPage}');
						}
		...
		//修改
		function toEdit(lbtId){
			 top.jzts();
			 var diag = new top.Dialog();
			 diag.Drag=true;
			 diag.Title ="编辑";
			 diag.URL = "<%=basePath%>background/lbt/toEdit.do?lbtId="+lbtId+"&tm="+new Date().getTime();
			 diag.Width = 450;
			 diag.Height = 500;
			 diag.Modal = true;				//有无遮罩窗口
			 diag. ShowMaxButton = true;	//最大化按钮
				...
				if(result) {
					...
					if(str==''){
						...
						if(msg == '确定要删除选中的数据吗?'){
							...
							$.ajax({
								type: "POST",
								url: '<%=basePath%>background/lbt/toBatchDelete.do?tm='+new Date().getTime(),
						    	data: {ids:str},
								dataType:'json',
		...
		function toExportExcel(){
			window.location.href='<%=basePath%>background/lbt/toExportExcel.do';
		}
		...
		function toUploadExcel(){
			 ...
			 diag.URL = '<%=basePath%>background/lbt/toUploadExcel.do';
			 diag.Width = 300;
			 diag.Height = 150;
			 diag.CancelEvent = function(){ //关闭事件
				 if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
