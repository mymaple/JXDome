...
<script type="text/javascript">
	...
	function parentReload(returnMsg,currentPage,showCount){
		if('change' == returnMsg){
			parent.location.href="<%=basePath%>background/menu/main.do?pId="+'${pd.pId}'+"&currentPage="+currentPage+"&showCount="+showCount;
		}
	}
</script>
</head>
<body class="no-skin">
						...
						<table id="simple-table" class="table table-striped table-bordered table-hover" style="margin-top:5px;">	
							<thead>
								<tr>
									...
									<th class="center">后台菜单代号</th>
									<th class="center">后台菜单名称</th>
									<th class="center">后台菜单类型</th>
									<th class="center">菜单数字标记</th>
									<th class="center">菜单链接</th>
							...
							<!-- 开始循环 -->	
							<c:choose>
								<c:when test="${not empty bgMenuList}">
									<c:if test="${RIGHTS.sele}">
									<c:forEach items="${bgMenuList}" var="bgMenu" varStatus="vs">
										<tr>
											<td class='center'>
												<label class="pos-rel"><input type='checkbox' name='ids' value="${bgMenu.menuId}" class="ace" /><span class="lbl"></span></label>
											</td>
											<td class='center' style="width: 30px;">${vs.index+1}</td>
											<td class='center'>${bgMenu.menuCode}</td>
											<td class='center'><a href="javascript:toSub('${bgMenu.menuId}')">${bgMenu.menuName}</a></td>
											<td class='center'><param:display type="bg_menuType" value="${bgMenu.menuType}"/></td>
											<td class='center'>${bgMenu.menuTag}</td>
											<td class='center'>${bgMenu.menuUrl}</td>
											<td class='center'><a onclick="toChangeMenuIcon('${bgMenu.menuId}')"><i class="${bgMenu.menuIcon}"></i></a></td>
											<td class='center'>${bgMenu.orderNum}</td>
											<td class="center">
												...
												<div class="hidden-sm hidden-xs btn-group">
													...
													<a class="btn btn-xs btn-success" title="编辑" onclick="toEdit('${bgMenu.menuId}');">
														<i class="ace-icon fa fa-pencil-square-o bigger-120" title="编辑"></i>
													</a>
													</c:if>
													<c:if test="${RIGHTS.del }">
													<a class="btn btn-xs btn-danger" onclick="toDelete('${bgMenu.menuId}');">
	...
	<script type="text/javascript">
		...
		function toAdd(){
			 ...
			 diag.Drag=true;
			 diag.Title ="新增";
			 diag.URL = "<%=basePath%>background/menu/toAdd.do?pId="+'${pd.pId}';
			 diag.Width = 450;
			 diag.Height = 355;
		...
		function toDelete(menuId){
			bootbox.confirm("确定要删除吗?", function(result) {
				if(result) {
					top.jzts();
					var url = "<%=basePath%>background/menu/toDelete.do?menuId="+menuId+"&tm="+new Date().getTime();
		...
		function toEdit(menuId){
			 ...
			 diag.Drag=true;
			 diag.Title ="编辑";
			 diag.URL = "<%=basePath%>background/menu/toEdit.do?menuId="+menuId+"&tm="+new Date().getTime();
			 diag.Width = 450;
			 diag.Height = 355;
			 diag.Modal = true;				//有无遮罩窗口
			 diag. ShowMaxButton = true;	//最大化按钮
				...
				if(result) {
					...
					if(str==''){
						...
						if(msg == '确定要删除选中的数据吗?'){
							...
							$.ajax({
								type: "POST",
								url: '<%=basePath%>background/menu/toBatchDelete.do?tm='+new Date().getTime(),
						    	data: {ids:str},
								dataType:'json',
		...
		function toExportExcel(){
			window.location.href='<%=basePath%>background/menu/toExportExcel.do';
		}
		...
		function toUploadExcel(){
			 ...
			 diag.URL = '<%=basePath%>background/menu/toUploadExcel.do?pId='+'${pd.pId}';
			 diag.Width = 300;
			 diag.Height = 150;
			 diag.CancelEvent = function(){ //关闭事件
				 if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
