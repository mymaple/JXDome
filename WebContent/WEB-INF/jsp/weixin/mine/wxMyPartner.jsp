...
<head>
	...
	<link rel="stylesheet" type="text/css" href="weui/gemo/css/mui.picker.css?${resultInfo.version}" />
	...
	<body>
		<div class=" warpthree clearfloat">
			...
			<c:forEach items="${map}" var="mymap">
				<div class="h-tit clearfloat box-s" style="margin-top:0;">
	    		<p><param:display type="com_appUserRoleEffectiveP" value="${mymap.key}"/></p>
	    		</div>	
				<c:choose>
				<c:when test="${not empty mymap.value}">
				<c:forEach items="${mymap.value}" var="list" varStatus="vs1">	
					<c:if test="${vs1.index == 0 }">
					
					<c:choose>
					<c:when test="${not empty list}">
					<c:forEach items="${list}" var="comAppUserRole" varStatus="vs2">	
					<div class="h-tit h-tit-c clearfloat box-s">
		    		<p><param:display type="com_appUserRoleType" value="${comAppUserRole.appUserRoleType}"/></p>
		    		</div>
			    		<c:choose>
						<c:when test="${not empty comAppUserRole.comAppUserList}">
		    			<div class="h-tab">
						<table width="100%" border="0">
						<c:forEach items="${comAppUserRole.comAppUserList}" var="comAppUser" varStatus="vs3">
						  <tr>
						    <td width="15%"><img src="<%=Const.BG_WEBSITE %>/${comAppUser.headImgSrc}"></td>
						    <td width="35%"><div class="qu-l">${comAppUser.appUserName}
						    <c:if test="${comAppUser.appUserId == userId}">(我)</c:if>
							    <p class="col1"><param:display type="com_appUserRoleEffectiveP" value="${comAppUserRole.appUserRoleId }"/></p>
							    <p class="col2">${comAppUser.phone}</p></div></td>
						    <td width="50%">
							    <c:if test="${comAppUser.appUserId != userId}">
							    <a href="tel:${comAppUser.phone}" class="qu-btn1 qu-bd1">拨打</a>
							    <a class="qu-btn1 qu-bd1" onclick="toTransfer('${comAppUser.appUserId}');">转赠</a></c:if>
							    </td>
						  </tr>
						</c:forEach>
						</table>
					...
					<c:if test="${vs1.index == 1 }">
					<c:choose>
					<c:when test="${not empty list}">
							<div class="h-tit h-tit-c clearfloat box-s">
				    		<p><param:display type="com_appUserRoleType" value="${list[0].appUserRoleType}"/></p>
				    		</div>
				    		<div class="h-tab">
							<table width="100%" border="0">
					<c:forEach items="${list}" var="subComAppUserRole" varStatus="vs4">	
						<c:choose>
